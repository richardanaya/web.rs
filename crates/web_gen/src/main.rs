...
struct Namespace {
    name: String,
    parts: Vec<TypescriptDefinitionFilePart>,
}

#[derive(Debug)]
enum TypescriptDefinitionFilePart {
    Comment(Comment),
    Interface(Interface),
...
#[derive(Debug)]
struct TypescriptDefinitionFile {
    parts: Vec<TypescriptDefinitionFilePart>,
}
...
fn parse_interface(input: &str) -> IResult<&str, TypescriptDefinitionFilePart> {
    ...
    let (input, _) = char('{')(input)?;
    let (input, functions) =
        map(opt(many0(parse_function)), |nodes| nodes.unwrap_or(vec![]))(input)?;
    let (input, _) = multispace0(input)?;
    let (input, _) = char('}')(input)?;
    Ok((
        input,
...
fn parse_namespace(input: &str) -> IResult<&str, TypescriptDefinitionFilePart> {
    ...
    let (input, _) = multispace0(input)?;
    let (input, _) = char('{')(input)?;
    let (input, parts) = map(opt(many0(parse_part)), |nodes| {
        nodes.unwrap_or(vec![])
    })(input)?;
    let (input, _) = multispace0(input)?;
    let (input, _) = char('}')(input)?;
...
fn parse(buffer: &[u8]) -> Result<TypescriptDefinitionFile, &'static str> {
...
fn main() {
    let input = include_bytes!("./test.d.ts");
