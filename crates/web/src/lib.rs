...
static ANIMATION_FRAME_EVENT_HANDLERS: Mutex<
    Option<HashMap<FunctionHandle, Box<dyn FnMut() + Send + 'static>>>,
> = Mutex::new(None);
...
pub fn request_animation_frame(handler: impl FnMut() + Send + 'static) {
    let function_ref = js!(r#"
        function(){
            const handler = () => {
    ...
    let function_handle = FunctionHandle(ExternRef {
        value: function_ref,
    });
    add_animation_frame_event_handler(function_handle, Box::new(handler));
}
